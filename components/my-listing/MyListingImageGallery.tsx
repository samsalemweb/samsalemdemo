...
interface MyListingImageGalleryProps {
    images: GalleryImage[];
    title: string;
}

export default function MyListingImageGallery({ images, title }: MyListingImageGalleryProps) {
    const [selectedIndex, setSelectedIndex] = useState(0);

    if (!images || images.length === 0) {
        return (
            <div className="w-full h-[400px] rounded-2xl flex items-center justify-center bg-gray-100">
    ...
    return (
        <div className="space-y-4">
            {/* Main Image */}
            <div className="relative w-full h-[350px] md:h-[500px] lg:h-[550px] rounded-2xl overflow-hidden bg-gray-50">
                ...
                <div className="flex gap-2 overflow-x-auto pb-2 scrollbar-hide">
                    {images.map((img, idx) => (
                        <button
                            ...
                            className={`relative shrink-0 w-20 h-16 md:w-24 md:h-18 rounded-xl overflow-hidden transition-all duration-300 ${
                                selectedIndex === idx
